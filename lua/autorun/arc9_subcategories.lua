...
hook.Add("PopulateWeapons", "GSR_SubCategories", function(pnlContent, tree, node)
    ...
    for _, node in pairs(tree:Root():GetChildNodes()) do
        ...
        -- If this category is not whitelisted, then skip it 
        local skip = true
        for _, cat in pairs(WhitelistedCategories) do
            if (node:GetText() == cat) then
                skip = false
                break
            end
        end
